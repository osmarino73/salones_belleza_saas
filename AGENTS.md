...
6. **Protocolo Obligatorio de Ingesta Web (`landing-html-injector`)**: SIEMPRE que se procese, genere o ingeste un sitio web o landing page de un salón/prospecto, se DEBE aplicar la habilidad `landing-html-injector` ([prospectHtmlInjector.ts](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/prospectHtmlInjector.ts)). Todos los botones de reserva (Header, Navbar, Hero, Footer) deben conducir automáticamente al agendador interactivo SaaS (`/reservar/:slug`), dejando exclusivamente el botón flotante verde para el canal de WhatsApp.
