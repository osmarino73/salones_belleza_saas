...
-82. **Corrección de Visibilidad y Anclaje Inferior del Hero en Móviles ([`prospectHtmlInjector.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/prospectHtmlInjector.ts))**:
    ...
    - **Solución Implementada**:
      1. Se removió la inyección de `overflow: hidden` y `display: flex` en los contenedores de scroll (`.hero-scroll-section`, `.canvas-sticky-wrapper`), preservando la arquitectura sticky del video-scroll.
      2. Se aplicó el anclaje inferior exclusivamente sobre las cajas contenedoras de texto (`.hero-content`, `.hero-text-box`, `.hero-bottom-content`, `.hero-overlay-content`, `.hero-caption`, `.hero-container-align`), manteniendo visible la modelo/fotogramas en la zona superior (`object-position: center 8%`).
      3. Se configuró tipografía compacta (`clamp(1.15rem, 4.2vw, 1.45rem)`) y degradado suave al `48%` de altura (`rgba(10, 10, 14, 0.92)`).
      4. Compilado y publicado exitosamente en GitHub (`b2d9600`).
...
-56. **Generador de Mensajes Persuasivos de WhatsApp Adaptados por Nicho (Barberías, Salones, Nails, Spas y Estética) ([`whatsappPitchGenerator.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/whatsappPitchGenerator.ts), [`SuperadminDashboardPage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/SuperadminDashboardPage.tsx), [`HomepageStudioModal.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/components/HomepageStudio/HomepageStudioModal.tsx), [`SKILL.md`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/.agents/skills/whatsapp-persuasive-copy/SKILL.md))**:
    ...
    - **Motor Modular `whatsappPitchGenerator.ts`**:
      - `detectBusinessNiche(businessName, category)`: Detección inteligente automática por categoría explícita o análisis de palabras clave regex con remoción de acentos (ej: `barber`, `barberia`, `corte`, `fade`, `nail`, `uñas`, `spa`, `facial`, `estetica`).
      - `generateStep1Pitch`: Saludo cordial con emoji de nicho (`💈`, `✨`, `💅`, `🧖‍♀️`, `🪄`), enlace demo a la web pública, aclaración de contenido 100% editable para el equipo y la pregunta de cierre con dolor agudo para disparar la respuesta de la dueña/dueño.
      - `generateStep2Pitch`: Oferta de activación de $89k de la Jornada de Lanzamiento adaptada en términos de imagen, colaboradores y afiches QR según el espacio (estaciones de barbería vs tocadores vs mesas de manicure vs cabinas).
      - `generateWelcomeCredentialsPitch`: Credenciales con instrucciones y términos de equipo adaptados.
    - **UI en Superadmin**: En el modal de publicación de prospectos, se integró un selector de nicho dinámico mediante pastillas interactivas (💈 Barbería, ✨ Salón, 💅 Uñas, 🧖‍♀️ Spa, 🪄 Estética) que permite alternar el tono del mensaje en vivo antes de copiar o enviar con el botón de 1 clic a WhatsApp. Se corrigió la etiqueta del Paso 2 a `$89k`.

-55. **Resolución de Error 404 en Contador de Visitas de Prospectos ([`supabase.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/supabase.ts), [`create_increment_views_rpc.sql`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/create_increment_views_rpc.sql))**:
    - **Causa Raíz**: Al cargar un sitio público (`/sitio/:slug`), `incrementProspectSiteViews` ejecutaba `supabase.rpc('increment_prospect_views')`. Al no existir la función RPC en la base de datos de Supabase, PostgREST arrojaba `POST /rest/v1/rpc/increment_prospect_views 404 (Not Found)` visible en consola.
    - **Solución Implementada**: Se rediseñó `incrementProspectSiteViews` para consultar e incrementar directamente `views_count` en la tabla `prospect_sites` (habilitada por la política RLS pública), eliminando la llamada RPC innecesaria y el error 404. Se creó además el script SQL opcional `create_increment_views_rpc.sql`.
...
-54. **Diagnóstico Inmune a CORS y Resolución Multi-Ruta de Fotogramas para Video-Scroll ([`SuperadminDashboardPage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/SuperadminDashboardPage.tsx), [`uploadFramesToR2.mjs`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/scripts/uploadFramesToR2.mjs))**:
    ...
    - **Contexto Activo**: Esta corrección asegura que el diagnóstico de carga en Cloudflare R2 sea fiable incluso en entornos de desarrollo local y subidas masivas desde múltiples directorios de origen.

-53. **Corrección Crítica: Carga de Fotogramas de Video-Scroll en Cloudflare R2 (`dp-beauty-nails` y sitios prospecto) ([`prospectHtmlInjector.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/prospectHtmlInjector.ts), [`PublicProspectSitePage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/PublicProspectSitePage.tsx), [`DashboardPage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/DashboardPage.tsx), [`SuperadminDashboardPage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/SuperadminDashboardPage.tsx), [`.env`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/.env))**:
    - **Causa Raíz del Fondo Negro**: En `/sitio/dp-beauty-nails`, las imágenes de la portada (Hero con canvas video scroll scrubbing y el poster estático `poster.webp`) no se visualizaban (pantalla negra) debido a:
      1. Las rutas en `raw_html` estaban guardadas como relativas locales `public/frames/desktop` y `public/frames/mobile`.
      2. `PublicProspectSitePage.tsx` pasaba `framesBaseUrl: /frames/${site.slug}` (ruta local sin host http/https).
      3. En producción (Netlify), `/frames/dp-beauty-nails/...` no existía físicamente en el build, por lo que Netlify devolvía el `index.html` (SPA) con código 200 pero tipo `text/html`. Al intentar renderizar texto HTML como imagen en `<canvas>` y `<img id="canvas-poster-fallback">`, el navegador fallaba silenciosamente y dejaba el canvas en negro puro.
      4. La variable `VITE_R2_CDN_URL` estaba definida solo en `.env.local` (ignorado en git), por lo que en entornos donde no se inyectara, `prospectHtmlInjector.ts` carecía de fallback a la URL pública de R2.
    - **Solución Implementada**:
      1. Se actualizó el registro en vivo de `dp-beauty-nails` en Supabase (`raw_html` y `business_data`) apuntando a la CDN de R2 (`https://pub-22e6e94a97b84b068f4217675926ef7f.r2.dev/frames/dp-beauty-nails/`).
      2. En `prospectHtmlInjector.ts`, se configuró `'https://pub-22e6e94a97b84b068f4217675926ef7f.r2.dev'` como fallback nativo universal para `VITE_R2_CDN_URL`.
      3. En `PublicProspectSitePage.tsx` y `DashboardPage.tsx`, se prioriza `tenant?.frames_cdn_url || bData.frames_cdn_url` antes de cualquier ruta relativa.
      4. En `SuperadminDashboardPage.tsx`, se añadió la reescritura automática de rutas relativas a URLs absolutas de R2 en `finalHtml` al publicar cualquier nuevo sitio con video scroll.
      5. Se añadió `VITE_R2_CDN_URL` en `.env`.

-52. **Monitor Forense de Cloudflare R2 y Previews en Superadmin ([`SuperadminDashboardPage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/SuperadminDashboardPage.tsx))**:
    ...
    - **Panel Dedicado de Gestión & Diagnóstico CDN**: Cuando se detecta video-scroll, se despliega un panel con degradado dorado que:
      1. Diagnostica en tiempo real con una petición `HEAD` a Cloudflare R2 si los fotogramas del negocio ya están en línea (`✅ Frames listos en CDN`) o si están pendientes (`⚠️ Pendiente de subida`).
      2. Ofrece botón de 1 clic para copiar el comando de subida: `npm run upload:frames <slug>`.
      3. Enlace directo al bucket `kowy-frames` en Cloudflare.
    - **Preset de 1 Clic para Sanus Spa**: Se agregó el botón de prueba rápida `🎬 Cargar: Sanus Spa (Video-Scroll)` junto a los presets de Kapa Spa y Luxus Beauty Spa.
    - **Persistencia en Supabase**: Los campos `has_video_scroll: true` y `frames_cdn_url` se guardan en `business_data`, permitiendo que la tabla de prospectos identifique los sitios con la insignia `🎬 Video Scroll HD`.

-52. **Integración de Cloudflare R2 y Script Automatizado de Subida de Fotogramas WebP ([`uploadFramesToR2.mjs`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/scripts/uploadFramesToR2.mjs), [`package.json`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/package.json), [`.env.example`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/.env.example), [`prospectHtmlInjector.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/prospectHtmlInjector.ts))**:
    - **Solución al Desafío de Almacenamiento & Ancho de Banda**: Ante la meta de crear ~10 prospectos diarios con video-scroll (72-144 fotogramas WebP por negocio, ~5MB), alojar imágenes en el repositorio Git inflaría el tamaño a gigabytes y Supabase Storage gratuito (2GB egress) colapsaría en menos de una semana.
    - **Cloudflare R2 con Egress Gratuito Ilimitado**: Se adoptó Cloudflare R2 (10 GB gratis de almacenamiento, 10M lecturas/mes y **$0 de costo de transferencia de datos / egress ilimitado**) sirviendo a través de la red global CDN de Cloudflare.
    - **Comando CLI de 1 Paso**: `npm run upload:frames <slug> [carpeta_origen]`. Detecta automáticamente los fotogramas en `public/frames/<slug>` o en carpetas personalizadas (ej. `document/<slug>/public/frames`), sube concurrentemente con lote de 8 workers vía `@aws-sdk/client-s3`, e inyecta headers de caché inmutable (`public, max-age=31536000, immutable`).
    - **Resolución Universal con Fallback Transparente**: `prospectHtmlInjector.ts` detecta `VITE_R2_CDN_URL` en variables de entorno. Si está configurado, reescribe dinámicamente las rutas de frames a la CDN de R2 (`https://<cdn-url>/frames/:slug/...`). Si no está configurado o en desarrollo local, mantiene fallback retrocompatible a `/frames/:slug/...`.

-51. **Soporte de Inyección SaaS para Plantillas con Canvas Video Scroll Scrubbing (Frames WebP en Header) ([`prospectHtmlInjector.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/prospectHtmlInjector.ts), [`sanusSpaSiteData.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/sanusSpaSiteData.ts), [`supabase.ts`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/supabase.ts), [`PublicProspectSitePage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/PublicProspectSitePage.tsx), [`DashboardPage.tsx`](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/pages/DashboardPage.tsx))**:
    - **Diagnóstico y Arquitectura**: Las nuevas plantillas de última generación (ej. `Sanus Spa`) utilizan un canvas HTML5 en el header que renderiza fotogramas WebP secuenciales sincronizados con el scroll de la página (`hero-scroll-section`, 300vh, 72 fotogramas para desktop y 72 para mobile).
    - **Prevención de Ruptura de `position: sticky`**: En el CSS inyectado (`beautyflow-prospect-reset`), se cambió `overflow-x: hidden` por `overflow-x: clip;`. En la especificación W3C, cualquier ancestro con `overflow-x: hidden` anula el comportamiento de `position: sticky; top: 0` en elementos hijos (`.canvas-sticky-wrapper`), mientras que `overflow-x: clip` previene el scroll horizontal sin desactivar el contexto de sticky.
    - **Reescritura Dinámica de Rutas de Fotogramas**: El inyector (`prospectHtmlInjector.ts`) ahora reescribe automáticamente referencias relativas como `public/frames/desktop` y `public/frames/mobile` hacia la CDN pública `/frames/:slug/desktop` y `/frames/:slug/mobile` (o el `framesBaseUrl` correspondiente).
    - **Preservación de Textos Secuenciales & Anclas**: Se asegura que los botones con anclas internas (`href="#servicios"`, `href="#nosotros"`) mantengan su scroll interno suave, mientras que los botones de llamada a la acción ("Agendar Cita", "Reservar Ahora", `.btn-header-cta`) redirijan de forma transparente al agendador interactivo `/reservar/:slug`.
    - **Registro de Sanus Spa**: Se alojaron los 144 fotogramas WebP en `public/frames/sanus-spa/` y se registró `SANUS_SPA_SITE_DATA` en el sistema para acceso inmediato en `/sitio/sanus-spa` y `/reservar/sanus-spa`.
    - **Solución a Pantalla Negra Inicial (Condición de Carrera en `preloadImages`)**: Se corrigió el error en la función `preloadImages()` del canvas donde `renderFrame(0)` solo se llamaba si la primera imagen en completar la descarga por red (`loadedCount === 1`) era casualmente el frame 0, y el poster se ocultaba prematuramente (`display: none`) con cualquier 6 frames cargados. Ahora: (1) `renderFrame(currentFrameIndex)` se ejecuta en cuanto se descarga el frame 0, (2) el poster de reserva permanece visible hasta que el canvas haya pintado físicamente su primer frame (`initialFrameDrawn`), y (3) se inyecta `<base href="/" />` para resolución universal de rutas en `srcDoc`.
    - **Escala 100% Full-Cover Edge-to-Edge**: Se eliminó el factor de reducción artificial `baseScale * 0.90` y el desplazamiento lateral `cw * 0.05` del template base en `renderFrame`. Ahora se aplica `scale = Math.max(cw / iw, ch / ih)` con centrado exacto `nx = (cw - nw) / 2` y `ny = (ch - nh) / 2`, logrando que los fotogramas del video llenen el 100% del ancho y alto de pantalla sin bordes negros ni márgenes vacíos.
