...
1. **Filosofía Mobile-First Obligatoria (>90% del Tráfico en Smartphones)**:
   ...
   - **Cuadrícula Doble Columna Mobile-First (2 Columnas)**: Tanto la sección de **Servicios** como la de **Colaboradores / Equipo** se renderizan en cuadrícula de 2 columnas (`repeat(2, 1fr)`) en pantallas móviles (≤768px y ≤480px), optimizando el espacio vertical, evitando scroll excesivo y ofreciendo una experiencia tipo catálogo prémium con tarjetas balanceadas.
   - **Fotografías Cuadradas (1:1) en Tarjetas**: Las imágenes de las tarjetas de servicios y catálogo adoptan una relación de aspecto cuadrada perfecta (`aspect-ratio: 1 / 1 !important; object-fit: cover; border-radius: 12px;`) para consistencia visual moderna.
   - **Unificación y Centrado de Encabezados de Sección**: Cualquier encabezado de sección dividido en múltiples columnas o asimétrico se normaliza automáticamente a un bloque vertical centrado (Eyebrow centrado + Título H2 centrado + Subtítulo centrado con contraste de texto legible `rgba(226, 232, 240, 0.88)`).
   - **Limpieza de Iconos/Medallas Desalineadas**: Se eliminan automáticamente elementos flotantes, medallas o arcos residuales entre la foto y el título de la tarjeta para garantizar una estética limpia y pulida.
