...
6. **Protocolo Obligatorio de Ingesta Web (`landing-html-injector`)**: SIEMPRE que se procese, genere o ingeste un sitio web o landing page de un salón/prospecto, se DEBE aplicar la habilidad `landing-html-injector` ([prospectHtmlInjector.ts](file:///c:/Users/Rio%20Belen/salones_belleza_saas/src/lib/prospectHtmlInjector.ts)). 
   - **Barra Superior en Móvil**: Debe ser SIEMPRE 100% transparente y limpia (`position: absolute; background: transparent; border: none; box-shadow: none;`), con el isotipo y nombre del negocio centrado flotando directamente sobre la foto del Hero sin fondos sólidos ni píldoras, ocultando botones de cabecera para no saturar la vista.
   - **Hero Inmersivo Móvil**: Fotografía al fondo con rostro libre en la parte superior y textos/botones apilados centrados del medio hacia abajo.
   - **Agendamiento SaaS**: Todos los botones de reserva (Header, Navbar, Hero, Footer) deben conducir automáticamente al agendador interactivo SaaS (`/reservar/:slug`), dejando exclusivamente el botón flotante verde para el canal de WhatsApp.
